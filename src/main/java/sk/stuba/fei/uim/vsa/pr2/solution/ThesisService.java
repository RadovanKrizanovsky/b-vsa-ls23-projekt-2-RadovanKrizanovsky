...
public class ThesisService
  extends AbstractThesisService<Student, Teacher, Thesis>
  implements PageableThesisService<Student, Teacher, Thesis> {
  ...
  public Student createStudent(Long aisId, String name, String email) {
    ...
    return utils.create(() ->
      Student.builder().aisId(aisId).name(name).email(email).build()
    );
  }
  ...
  public Student updateStudent(Student student) {
    if (student == null) throw new IllegalArgumentException(
      "Provided student must not be null"
    );
    if (student.getAisId() == null) throw new IllegalArgumentException(
      "Provided student.aisId must not be null"
    );
  ...
  public List<Student> getStudents() {
    return utils.findByNamedQuery(
      Student.FIND_ALL_QUERY,
      Student.class,
  ...
  public Teacher updateTeacher(Teacher teacher) {
    if (teacher == null) throw new IllegalArgumentException(
      "Provided teacher must not be null"
    );
    if (teacher.getAisId() == null) throw new IllegalArgumentException(
      "Provided teacher.aisId must not be null"
    );
  ...
  public Teacher deleteTeacher(Long id) {
    ...
    utils.execute(
      "delete from Thesis t where t.supervisor.aisId = :teacherId",
      Collections.singletonMap("teacherId", id),
      true
    );
  ...
  @Override
  public Thesis makeThesisAssignment(
    Long supervisor,
    String title,
    String type,
    String description
    ...
    if (supervisor == null) throw new IllegalArgumentException(
      "Supervisor id must not be null"
    );
    return utils.create(() -> {
      Teacher teacher = this.getTeacher(supervisor);
      if (teacher == null) throw new IllegalArgumentException(
        "Provided supervisor teacher with id '" +
        supervisor +
        "' has not been found!"
      );
  ...
  public Thesis assignThesis(Long thesisId, Long studentId) {
    if (thesisId == null) throw new IllegalArgumentException(
      "Thesis Id must not be null"
    );
    if (studentId == null) throw new IllegalArgumentException(
      "Student id must not be null"
    );
    Thesis thesis = getThesis(thesisId);
    if (thesis == null) throw new IllegalArgumentException(
      "Thesis with id '" + thesisId + "' has not been found!"
    );
    if (
      thesis.getStatus() != ThesisStatus.FREE_TO_TAKE
    ) throw new IllegalStateException(
      "Thesis is not in the state to be assigned to a student"
    );
    if (
      LocalDate.now().isAfter(asLocalDate(thesis.getDeadline()))
    ) throw new IllegalStateException(
      "Thesis cannot be assigned to a student after the deadline on " +
      thesis.getDeadline().toString()
    );
  ...
  public Thesis submitThesis(Long thesisId) {
    ...
    if (
      ...
      "Thesis cannot be submitted after the deadline on " +
      thesis.getDeadline().toString()
    );
    if (
      thesis.getStatus() != ThesisStatus.IN_PROGRESS
    ) throw new IllegalStateException(
      "Thesis is not in the state to be submitted"
    );
    if (thesis.getAuthor() == null) throw new IllegalStateException(
      "Thesis cannot be submitted if it hasn't been assigned to a student"
    );
  ...
  public Thesis getThesisByStudent(Long studentId) {
    List<Thesis> results = utils.findByNamedQuery(
      Thesis.FIND_ALL_BY_AUTHOR,
      Thesis.class,
  ...
  public Thesis updateThesis(Thesis thesis) {
    if (thesis == null) throw new IllegalArgumentException(
      "Provided thesis must not be null"
    );
    if (thesis.getId() == null) throw new IllegalArgumentException(
      "Provided thesis.id must not be null"
    );
    return utils.update(thesis);
  }
  ...
  @Override
  public Page<Student> findStudents(
    ...
    List<Student> students = utils.findByQuery(em ->
      em
        .createQuery(finalQuery, Student.class)
  ...
  @Override
  public Page<Teacher> findTeachers(
    Optional<String> name,
    ...
    List<Teacher> teachers = utils.findByQuery(em ->
      em
        .createQuery(finalQuery, Teacher.class)
        .setMaxResults(pageable.getPageSize())
        .setFirstResult(pageable.getPageSize() * pageable.getPageNumber())
    );
  ...
  public Page<Thesis> findTheses(
    ...
    List<Thesis> theses = utils.findByQuery(em -> {
      TypedQuery<Thesis> q = em
        .createQuery(finalQuery, Thesis.class)
        .setMaxResults(pageable.getPageSize())
        .setFirstResult(pageable.getPageSize() * pageable.getPageNumber());
  ...
  @Override
  public Optional<Teacher> getTeacherByHisEmail(String hisEmail) {
    EntityManager em = emf.createEntityManager();
    Query query = em.createQuery(
      "select t from Teacher t where t.email = :email"
    );
    query.setParameter("email", hisEmail);
    return query.getResultStream().findFirst();
  }

  @Override
  public Optional<Student> getStudentByHisEmail(String hisEmail) {
    EntityManager em = emf.createEntityManager();
    Query query = em.createQuery(
  ...
  public Thesis makeThesisAssignment(
    ...
    List<Thesis> supervisedTheses = new ArrayList<>();
    return utils.create(() -> {
      LocalDate now = LocalDate.now();

      //REMOVE
      Teacher tempTeacher = utils.create(() ->
        Teacher
          .builder()
          .aisId(5194189496L)
          .name("dnkdjakbdabdk")
          .email("dnkdjakbdabdk")
          .department("dnkdjakbdabdk")
          .institute("dnkdjakbdabdk")
          .build()
      );
