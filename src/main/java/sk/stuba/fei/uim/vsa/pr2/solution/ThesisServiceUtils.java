...
public class ThesisServiceUtils {
  ...
  public <R> R create(Supplier<R> createFunction) {
    ...
    try {
      ...
      log.info(
        "New entity '" + entity.getClass().getSimpleName() + "' was created"
      );
      return entity;
    } catch (Exception e) {
      log.error(
        "Persist operation of entity '" +
        (entity != null ? entity.getClass().getSimpleName() : "unknown") +
        "' has failed due to: " +
        e.getMessage(),
        e
      );
      if (manager.getTransaction().isActive()) manager
        .getTransaction()
        .rollback();
      return null;
    } finally {
  ...
  public <R> List<R> findByQuery(
    Function<EntityManager, TypedQuery<R>> querySupplier
  ) {
  ...
  public <R> List<R> findByNamedQuery(
    String queryName,
    Class<R> clazz,
    Map<String, Object> parameters
  ) {
    if (
      queryName == null || queryName.isEmpty() || clazz == null
    ) return Collections.emptyList();
    EntityManager manager = this.emf.createEntityManager();
    try {
  ...
  public Long getCount(String query, Map<String, Object> parameters) {
    ...
    try {
      ...
      log.error(
        "Could not count entities with query '" +
        query +
        "' " +
        (
          parameters != null
